...
## 오늘 느낀점
많이 어려운데 계속 반복해서 보고 또 보니까 이제는 어떤식으로 Redux가 작동하는지 감이 잡힌다. 아직은 내가 생각하고 주르륵 코드를 짤수는 없지만 이렇게 반복하다보면은 액션 타입, 액션 생성함수, 리듀서 정도는 착착 쓰지 않을까 싶다. 부족한 부분은 본인 스스로가 제일 잘 알기 때문에 열심히 공부 하고 잊지 않기를 바란다.